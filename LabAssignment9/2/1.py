...
def exp(rv):
    ...
    return np.array([[np.cos(theta) + rv[0] * rv[0] * (1 - np.cos(theta)), rv[0] * rv[1] * (1 - np.cos(theta)) - rv[2] * np.sin(theta), rv[0] * rv[2] * (1 - np.cos(theta)) + rv[1] * np.sin(theta)],
                  [rv[1] * rv[0] * (1 - np.cos(theta)) + rv[2] * np.sin(theta), np.cos(theta) + rv[1] * rv[1] * (1 - np.cos(theta)), rv[1] * rv[2] * (1 - np.cos(theta)) - rv[0] * np.sin(theta)],
                  [rv[2] * rv[0] * (1 - np.cos(theta)) - rv[1] * np.sin(theta), rv[2] * rv[1] * (1 - np.cos(theta)) + rv[0] * np.sin(theta), np.cos(theta) + rv[2] * rv[2] * (1 - np.cos(theta))]])
...
def log(R):
    ...
    return normalized(np.array([v1,v2,v3])) * theta

def slerp(R1, R2, t):
    temp = np.transpose(R1) @ R2
    return R1 @ exp(t * log(temp))
